...
def directory_link(directory: Path):
    data = {}
    ...
    with (open('result.json', 'w') as f_json,
        ...
        for dir_path, dir_name, file_name in os.walk(directory):
            ...
            for dir in dir_name:
                ...
                data[dir_path].update({dir: {'object_size': size, 'object_type': 'directory','parent_directory': dir_path.split('\\')[-2]}})
                rows.append({'object_name': dir, 'path': dir_path, 'object_size': size, 'object_type': 'directory', 'parent_directory': dir_path.split('\\')[-2]})
            for i in file_name:
                size = get_size(dir_path + '/' + i)
                data[dir_path].update({i: {'object_size': size, 'object_type': 'file', 'parent_directory': dir_path.split('\\')[-1]}})
                rows.append({'object_name': i, 'path': dir_path, 'object_size': size, 'object_type': 'file', 'parent_directory': dir_path.split('\\')[-1]})
            #print(f'{dir_path = }\n{dir_name = }\n{file_name = }\n')
